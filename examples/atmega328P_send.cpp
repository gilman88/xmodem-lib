 ...
 * 
 * You can test this over your USB port using lrzsz: `stty -F /dev/ttyUSB0 1200 && rx -vX /path/to/save/flile > /dev/ttyUSB0 < /dev/ttyUSB0`
...
void setup() {
  ...
  printSd();
  mySerial.println("Enter file to send name?");
  mySerial.flush();

  fileNameBuffer = mySerial.readStringUntil('\n');
  if(fileNameBuffer.length() ==0)resetFunc();
  
  mySerial.println("read from: "+fileNameBuffer);
  mySerial.flush();
...
void loop() {
  mySerial.println(myXModem.sendFile(fileNameBuffer));
  resetFunc();
}